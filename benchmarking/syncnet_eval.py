# syncnet_eval.py (standard SyncNet, not SyncNet_color)

import torch
import torch.nn.functional as F
import numpy as np
import os
from benchmarking_utils import get_frame_audio_pairs
...
class SyncEvaluator:
    def __init__(self, checkpoint_path, syncnet_config, weight_type = torch.float32):
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        syncnet_config = OmegaConf.load(syncnet_config)
        self.model = SyncNet(OmegaConf.to_container(syncnet_config.model)).to(self.device)

        checkpoint = torch.load(checkpoint_path, map_location=self.device)
        self.model.load_state_dict(checkpoint["state_dict"])
        self.model.to(dtype=weight_type)
...
if __name__ == '__main__':
    ...
    score = evaluator.evaluate(args.video, args.audio)
    print(f'SyncNet Score: {score:.4f}')
