...
if [ "$mode" = "normal" ]; then
    ...
    result_dir="/MuseTalk/results/finetuned_2"
else
...
if [ "$version" = "v1.0" ]; then
    model_dir="/MuseTalk/models/musetalk"
    unet_model_path="$model_dir/pytorch_model.bin"
    unet_config="$model_dir/musetalk.json"
    version_arg="v1"
elif [ "$version" = "v1.5" ]; then
    model_dir="/MuseTalk/exp_out/stage2_exp_3/dave_custom_data"
    unet_model_path="$model_dir/unet-5000.pth"
